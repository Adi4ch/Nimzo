...
class NimzoWallet {
  ...
  factory NimzoWallet.fromMap(Map<String, dynamic> map) => NimzoWallet(id: map['id'] as String, userId: map['user_id'] as String? ?? '', balance: map['balance'] as int? ?? 0, transactions: (map['transactions'] as List<dynamic>? ?? []).map((item) => WalletTransaction.fromMap(Map<String, dynamic>.from(item as Map))).toList());
