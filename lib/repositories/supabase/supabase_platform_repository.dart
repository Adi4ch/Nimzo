...
class SupabasePlatformRepository implements PlatformRepository {
  ...
  Future<List<Map<String, dynamic>>> getDailyTasks() async {
    ...
    final tasks = await _client.from('daily_tasks').select().eq('active', true).order('title');
    final progress = await _client.from('daily_task_progress').select().eq('user_id', userId).eq('task_date', DateTime.now().toIso8601String().substring(0, 10));
    final byTask = {for (final row in progress) row['task_id'].toString(): Map<String, dynamic>.from(row)};
  ...
  Future<List<Map<String, dynamic>>> getAchievements() async {
    final userId = _client.auth.currentUser?.id;
    if (userId == null) return const [];
    final achievements = await _client.from('achievements').select().order('title');
    final earned = await _client.from('user_achievements').select().eq('user_id', userId);
    final byAchievement = {for (final row in earned) row['achievement_id'].toString(): Map<String, dynamic>.from(row)};
    return achievements.map((row) {
      ...
      achievement['progress'] = current == null ? 0 : achievement['target'];
      achievement['complete'] = current != null;
  ...
  Future<Map<String, dynamic>> purchaseMallItem(String itemId) async {
    final row = await _client.rpc('purchase_mall_item', params: {'target_item': itemId, 'request_key': '${_client.auth.currentUser?.id}:${itemId}:${DateTime.now().microsecondsSinceEpoch}'});
    return Map<String, dynamic>.from(row as Map);
  }

  @override
  Future<Map<String, dynamic>> purchaseVip(String levelId) async {
    final row = await _client.rpc('purchase_vip', params: {'target_level': levelId, 'request_key': '${_client.auth.currentUser?.id}:vip:$levelId:${DateTime.now().microsecondsSinceEpoch}'});
    return Map<String, dynamic>.from(row as Map);
  }

  @override
  Future<Map<String, dynamic>> equipMallItem(String inventoryId) async => Map<String, dynamic>.from(await _client.rpc('equip_mall_item', params: {'target_inventory': inventoryId}) as Map);

  @override
  Future<Map<String, dynamic>> unequipMallItem(String inventoryId) async => Map<String, dynamic>.from(await _client.rpc('unequip_mall_item', params: {'target_inventory': inventoryId}) as Map);

  @override
  Future<void> claimDailyTask(String taskId) async { await _client.rpc('claim_daily_task', params: {'target_task': taskId}); }

  @override
  Future<void> claimAchievement(String achievementId) async { await _client.rpc('claim_achievement_reward', params: {'target_achievement': achievementId}); }
