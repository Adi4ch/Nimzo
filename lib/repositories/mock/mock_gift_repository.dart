...
class MockGiftRepository implements GiftRepository {
  ...
  @override
  Future<Map<String, int>> getInventory() async => Map<String, int>.from(inventory);

  @override
  Future<Set<String>> getFavorites() async => Set<String>.from(favorites);

  @override
  Future<List<GiftEvent>> getRecent({int limit = 20}) async => recent.take(limit).toList();

  @override
  Future<void> setFavorite(String giftId, bool favorite) async { favorite ? favorites.add(giftId) : favorites.remove(giftId); }
  ...
  Future<GiftEvent> sendGift({required String roomId, required String giftId, String? receiverId, required int quantity, required String idempotencyKey}) async {
    ...
    if ((inventory[giftId] ?? 0) < quantity) throw StateError('Insufficient gift inventory.');
    ...
    inventory[giftId] = (inventory[giftId] ?? 0) - quantity;
