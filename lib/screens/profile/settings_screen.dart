...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: loading ? const Center(child: CircularProgressIndicator(color: mint)) : ListView(padding: nimzoPagePadding, children: [
          ...
          ListTile(contentPadding: EdgeInsets.zero, leading: const Icon(Icons.block_outlined), title: const Text('Blocked users'), onTap: _showBlockedUsers),
  ...
  Future<void> _showBlockedUsers() async {
    ...
    if (!mounted) return;
    showModalBottomSheet<void>(context: context, builder: (_) => SafeArea(child: ListView(padding: nimzoPagePadding, children: [const Text('Blocked users', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800)), const SizedBox(height: 12), if (blocked.isEmpty) const Text('No blocked users.') else ...blocked.map((id) => ListTile(title: Text(id), trailing: TextButton(onPressed: () async { await social.unblockUser(id); if (mounted) Navigator.pop(context); }, child: const Text('Unblock'))))])));
  }
}