  ...
  Widget build(BuildContext context) => DefaultTabController(
        ...
        child: Scaffold(
          appBar: AppBar(
            ...
            bottom: const TabBar(tabs: [Tab(text: 'VIP'), Tab(text: 'Mall'), Tab(text: 'Tasks'), Tab(text: 'Rankings')]),
          ),
          body: const TabBarView(children: [VipTab(), MallTab(), TasksTab(), RankingsTab()]),
        ),
      );
}
  ...
  Widget build(BuildContext context) => FutureBuilder<List<dynamic>>(
        ...
        builder: (context, snapshot) {
          ...
          return ListView(padding: nimzoPagePadding, children: [
            _HeroPanel(icon: Icons.workspace_premium, title: 'VIP ${current?['name'] ?? 'Member'}', subtitle: 'Unlock more ways to stand out in Nimzo.', child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Progress to next level', style: TextStyle(color: muted)), Text('${(progress * 100).round()}%', style: const TextStyle(fontWeight: FontWeight.w800, color: mint))]),
              const SizedBox(height: 8),
              LinearProgressIndicator(value: progress, minHeight: 9, borderRadius: BorderRadius.circular(10), color: mint, backgroundColor: Colors.white),
            ])),
...
class _MallTabState extends State<MallTab> {
  ...
  Widget build(BuildContext context) => FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator(color: mint));
          if (snapshot.hasError) return _EmptyState('The Mall is temporarily unavailable.');
          final allItems = snapshot.data ?? const <Map<String, dynamic>>[];
          final categories = ['All', ...allItems.map((item) => item['category'] as String).toSet()];
          final visible = category == 'All' ? allItems : allItems.where((item) => item['category'] == category).toList();
          return ListView(padding: nimzoPagePadding, children: [
            const _SectionIntro(title: 'Make your space yours', subtitle: 'Frames, themes and effects purchased with virtual coins.'),
            SizedBox(height: 42, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: categories.length, separatorBuilder: (_, __) => const SizedBox(width: 8), itemBuilder: (_, index) => ChoiceChip(label: Text(categories[index]), selected: category == categories[index], onSelected: (_) => setState(() => category = categories[index])))),
            const SizedBox(height: 12),
            if (visible.isEmpty) const _EmptyState('Nothing in this category yet.') else ...visible.map((item) => Card(child: ListTile(leading: CircleAvatar(backgroundColor: lightMint, child: Icon(_icon(item['icon'] as String?), color: mint)), title: Text(item['name'] as String, style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text('${item['description']}\n${item['category']}'), isThreeLine: true, trailing: FilledButton(onPressed: () => purchase(item), child: Text('${item['price_coins']}'))))),
            const SizedBox(height: 16),
            OutlinedButton.icon(onPressed: () => _showInventory(context), icon: const Icon(Icons.inventory_2_outlined), label: const Text('View my inventory')),
          ]);
        },
      );
...
class _RankingsTabState extends State<RankingsTab> {
  ...
  @override
  Widget build(BuildContext context) => FutureBuilder<List<Map<String, dynamic>>>(
        future: RepositoryFactory.platform().getRankings(period: period, kind: kind),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator(color: mint));
          final rows = snapshot.data ?? const <Map<String, dynamic>>[];
          return ListView(padding: nimzoPagePadding, children: [
            SegmentedButton<String>(segments: const [ButtonSegment(value: 'daily', label: Text('Daily')), ButtonSegment(value: 'weekly', label: Text('Weekly')), ButtonSegment(value: 'monthly', label: Text('Monthly')), ButtonSegment(value: 'all', label: Text('All time'))], selected: {period}, onSelectionChanged: (value) => setState(() => period = value.first)),
            const SizedBox(height: 10),
            DropdownButtonFormField<String>(value: kind, decoration: const InputDecoration(labelText: 'Ranking by'), items: const [DropdownMenuItem(value: 'users', child: Text('Users')), DropdownMenuItem(value: 'gift_senders', child: Text('Gift senders')), DropdownMenuItem(value: 'gift_receivers', child: Text('Gift receivers')), DropdownMenuItem(value: 'hosts', child: Text('Hosts')), DropdownMenuItem(value: 'rooms', child: Text('Rooms'))], onChanged: (value) => setState(() => kind = value!)),
            const SizedBox(height: 16),
            if (snapshot.hasError || rows.isEmpty) const _EmptyState('No rankings for this period yet.') else ...rows.map((row) => Card(child: ListTile(leading: CircleAvatar(backgroundColor: row['current'] == true ? lightMint : Colors.grey.shade100, child: Text('${row['rank']}')), title: Text(row['name']?.toString() ?? row['subject_id']?.toString() ?? 'Nimzo member', style: const TextStyle(fontWeight: FontWeight.w800)), trailing: Text('${row['score']} pts', style: const TextStyle(color: mint, fontWeight: FontWeight.w800))))),
          ]);
        },
      );
}
